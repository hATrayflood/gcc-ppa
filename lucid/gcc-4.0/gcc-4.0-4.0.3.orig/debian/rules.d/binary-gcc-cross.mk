...
ifeq ($(biarch),yes)
	...
	dh_link -p$(p_gcc) \
	  ...
	  /$(PF)/$(DEB_TARGET_GNU_TYPE)/$(lib64)/libgcc_s.so.$(GCC_SONAME) /$(gcc_lib_dir)/64/libgcc_s.so
endif
ifeq ($(biarch32),yes)
	...
	dh_link -p$(p_gcc) \
	  ...
	  /$(PF)/$(DEB_TARGET_GNU_TYPE)/lib32/libgcc_s.so.$(GCC_SONAME)  /$(gcc_lib_dir)/32/libgcc_s_32.so \
	  /$(PF)/$(DEB_TARGET_GNU_TYPE)/lib32/libgcc_s.so.$(GCC_SONAME)  /$(gcc_lib_dir)/32/libgcc_s.so
endif

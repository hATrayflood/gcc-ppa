...
$(binary_stamp)-libstdcxx: $(install_stamp)
	...
	find $(d_dbg)
	tar -C $(d_lib) -c -f - usr/lib/debug | tar -v -C $(d_dbg) -x -f -
	rm -rf $(d_lib)/usr/lib/debug
...
$(binary_stamp)-lib64stdcxx: $(install_stamp)
	...
	find $(d_lib64)
	tar -C $(d_lib64) -c -f - usr/lib/debug | tar -v -C $(d_dbg64) -x -f -
	rm -rf $(d_lib64)/usr/lib/debug

	debian/dh_doclink -p$(p_lib64) $(p_base)
	debian/dh_doclink -p$(p_dbg64) $(p_base)

	debian/dh_rmemptydirs -p$(p_lib64)
	dh_compress -p$(p_lib64) -p$(p_dbg64)
	dh_fixperms -p$(p_lib64) -p$(p_dbg64)
...
#dpkg-shlibdeps: failure: ldd on `debian/lib64gcc1/lib64/libgcc_s.so.1' gave error exit status 1
	...
	dh_installdeb -p$(p_lib64) -p$(p_dbg64)
	dh_md5sums -p$(p_lib64) -p$(p_dbg64)
	dh_builddeb -p$(p_lib64) -p$(p_dbg64)
...
$(binary_stamp)-lib32stdcxx: $(install_stamp)
	...
	find $(d_lib32)
	tar -C $(d_lib32) -c -f - usr/lib/debug | tar -v -C $(d_dbg32) -x -f -
	rm -rf $(d_lib32)/usr/lib/debug

	debian/dh_doclink -p$(p_lib32) $(p_base)
	debian/dh_doclink -p$(p_dbg32) $(p_base)

	debian/dh_rmemptydirs -p$(p_lib32)
	dh_compress -p$(p_lib32) -p$(p_dbg32)
	dh_fixperms -p$(p_lib32) -p$(p_dbg32)
	dh_gencontrol -p$(p_lib32) -- -v$(DEB_VERSION) $(common_substvars)
	dh_gencontrol -p$(p_dbg32) -- -v$(DEB_VERSION) $(common_substvars)

	dh_installdeb -p$(p_lib32) -p$(p_dbg32)
	dh_md5sums -p$(p_lib32) -p$(p_dbg32)
	dh_builddeb -p$(p_lib32) -p$(p_dbg32)

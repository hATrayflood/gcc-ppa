...
dirs_gcj = \
	...
	$(gcc_lexec_dir)
files_gcj = \
	$(PF)/bin/{gcj,gjar,gc-analyze,gjarsigner,gcjh,gjavah,gnative2ascii,grmic,gtnameserv,jv-convert,jcf-dump}$(pkg_ver) \
	$(PF)/share/man/man1/{gjar,gjarsigner,gcjh,gjavah,gnative2ascii,gtnameserv}$(pkg_ver).1 \
...
ifeq ($(with_standalone_gcj),yes)
  ...
  files_gcj += \
	...
	$(shell for h in \
		    README features.h \
		    {decfloat,float,iso646,limits,std*,syslimits,unwind,varargs}.h \
...
ifeq ($(with_separate_libgcj),yes)
  ifeq ($(PKGSOURCE),gcj-$(BASE_VERSION))
	...
	echo "libgcj_bc 1 libgcj-bc (>= 4.2.1-1)" >> debian/$(p_jlib)/DEBIAN/shlibs
...
#	dh_makeshlibs -p$(p_jqt) -V '$(p_jqt) (>= $(DEB_GCJ_SOVERSION))'
	...
	dh_compress -p$(p_gij) -p$(p_jlib) -p$(p_jlibx) $(peer_pkgs)
	dh_fixperms -p$(p_gij) -p$(p_jlib) -p$(p_jlibx) $(peer_pkgs)
...
#		$(d_jlibx)/usr/share/lintian/overrides/$(p_jlibx)
	...
	dh_installdeb -p$(p_gij) -p$(p_jlib) -p$(p_jlibx) $(peer_pkgs)
	dh_md5sums -p$(p_gij) -p$(p_jlib) -p$(p_jlibx) $(peer_pkgs)
	dh_builddeb -p$(p_gij) -p$(p_jlib) -p$(p_jlibx) $(peer_pkgs)
...
ifneq ($(GFDL_INVARIANT_FREE),yes)
	...
	dh_fixperms -p$(p_gcj)
	dh_shlibdeps -p$(p_gcj) -l$(d_lib)/$(PF)/$(libdir):$(d_jlib)/$(PF)/lib -Xecj1
...
ifeq ($(with_static_java),yes)
	...
	$(builddir)/gcc/xgcc -B$(builddir)/gcc/ -shared -fpic \
		...
		-o $(d_jdev)/$(gcc_lib_dir)/libgcj_bc.so \
